...
function OnScreenKeyboard({ onKey, onBackspace, onEnter, onClose }) {
  ...
  const key = (label, onTap, flex = 1, extra = {}) => (
    <button key={label} type="button" onMouseDown={(e) => e.preventDefault()} onClick={onTap}
      style={{ flex, minWidth: 0, height: 68, borderRadius: 12, border: `1px solid ${C.line}`, background: C.panel2, color: C.text, fontSize: 26, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', userSelect: 'none', ...extra }}>
      {label}
    </button>
  );
  const toggleKey = key(mode === 'num' ? 'ABC' : '123', () => setMode((m) => (m === 'num' ? 'abc' : 'num')), 1.4, { background: C.blue, color: '#fff', fontSize: 20 });
  const bottomRow = (
    <div style={{ display: 'flex', gap: 10 }}>
      {key(ARABIC ? 'إغلاق' : 'Hide', onClose, 1.4, { fontSize: 19 })}
      {key('␣', () => onKey(' '), 4)}
      {key(ARABIC ? 'دخول' : 'Enter', onEnter, 2, { background: C.green, color: C.accentText, fontSize: 20 })}
    </div>
  );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
        {[['1', '2', '3'], ['4', '5', '6'], ['7', '8', '9']].map((r, i) => (
          <div key={i} style={{ display: 'flex', gap: 10 }}>{r.map(cell)}</div>
        ))}
        <div style={{ display: 'flex', gap: 10 }}>
          {toggleKey}
          {cell('0')}
